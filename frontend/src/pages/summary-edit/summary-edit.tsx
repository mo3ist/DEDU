...
const SummaryEdit: React.FC<Props> = () => {
	...
	const checkNewTags = (tags: string[]): boolean => {
		return tags.filter(is_new => !summary?.tagSet?.edges.map(tag => tag?.node?.title).includes(is_new)).length > 0
	}

	return (
		<div className="grid grid-cols-1 gap-4 bg-secondary-100 p-2 rtl relative h-full rounded-lg main-margin">
			{mutationData && <Success redirectUrl={`/courses/${courseCode}/summary/`} />}
			...
			<GenericCreation 
				...
				onSubmit={({ title, body, tags }) => {
					// TODO: Tag inequality
					if (title !== summary?.title || body !== summary?.body || checkNewTags(tags!)) {
						editSummary({
							variables: {
